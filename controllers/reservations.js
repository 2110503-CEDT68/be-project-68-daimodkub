...
exports.getReservations = async (req, res, next) => {
    ...
    if (req.user.role !== 'admin') {
        ...
        } else if (req.query.myReservations === 'true') {
             query = Reservation.find({ user: req.user.id }).populate({
                path: 'massageShop',
                select: 'name address tel opentime closetime'
            });
...
exports.addReservation = async (req, res, next) => {
    try {
        ...
        // Assuming req.body.date is passed in exactly as ISO string, to check for identical timestamp:
        const overlappingReservation = existedReservations.find(r => 
            r.date.getTime() === new Date(req.body.date).getTime()
        );
        if(overlappingReservation) {
            return res.status(400).json({ success: false, message: `The user is already booked for another reservation at this time` });
        }
